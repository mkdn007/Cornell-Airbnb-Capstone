...
def score_confidence(rel_unc, seg_mape):
    # FIX (see model v3/proposed-confidence-fix/CONFIDENCE_FIX_WRITEUP.md):
    # the original stability = clip(1 - rel_unc/0.25, 0, 1) assumed
    # rel_unc could approach 0, but this model's actual uncertainty floor
    # sits at ~0.22-0.28, right next to that threshold, so nearly every
    # listing clipped to stability=0 and confidence collapsed to "Low"
    # for all 9,752 listings. Within-segment variation in rel_unc is also
    # very tight relative to its own scale (std of ~0.012-0.016 against a
    # mean of ~0.23-0.28), so a fixed or percentile-ratio threshold can't
    # create real separation no matter what denominator is chosen, it was
    # tested directly against this data before landing on this fix.
    # Using each listing's within-segment percentile rank instead
    # guarantees stability spreads across its full 0-1 range regardless
    # of how tightly clustered the raw values are, since rank is uniform
    # by construction. Everything else (weights, model_quality, label
    # cutoffs) is unchanged from the original.
